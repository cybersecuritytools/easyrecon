...
from utils.merger import save_to_file, _strip_ansi, _normalize_url
...
def run_live_phase(
    ...
    if urls:
        ...
        normalized = []
        for url in urls:
            clean = _strip_ansi(url)
            if clean:
                norm = _normalize_url(clean, target)
                if norm.startswith(("http://", "https://")):
                    normalized.append(norm)
        normalized = sorted(set(normalized))
        live_urls = _filter_live_urls(normalized, config, processed_dir)
...
def _probe_subdomains(
    ...
    try:
        ...
        lines = [_strip_ansi(l) for l in result.stdout.splitlines() if l.strip()]
        lines = [l for l in lines if l]

        save_to_file(lines, output_dir / "live_subdomains.txt")
        print_tool_result("httpx", "success" if lines else "empty", len(lines), elapsed)
...
def _filter_live_urls(
    ...
    """Run httpx on normalized URLs to filter live ones."""
    import time
    ...
    try:
        ...
        for line in result.stdout.splitlines():
            ...
            if any(code in clean for code in ["[200]", "[301]", "[302]", "[403]", "[401]", "[405]", "[500]"]):
                live_lines.append(clean)
            elif clean.startswith(("http://", "https://")):
                live_lines.append(clean)

        save_to_file(live_lines, output_dir / "live_urls.txt")
        print_tool_result("httpx (urls)", "success" if live_lines else "empty", len(live_lines), elapsed)
        return live_lines
